...
def render_branches(self, frame):
    ...
    for key, branch_data in branches_data.items():
        ...
        ci_cd = ((b_name == 'master' or b_name == 'development' or b_name == 'release')  and self.get_data('current_repo.ci_cd'))
        ...
        if ci_cd or active_branch or is_dirty:
            ...
            if len(b_diff) > 0 and b_name != 'master' and active_branch:
                mbutton = ttk.Button(frame, text="Merge", command=lambda  b_name=b_name: self.action_merge_branch(b_name))
                if (is_dirty):
            ...
            abutton =  ttk.Button(frame, text="Activate", command=lambda  b_name=b_name: self.action_switch_branch(b_name))
            dbutton = ttk.Button(frame, text="Delete", command=lambda  b_name=b_name: self.action_delete_branch(b_name))
    ...
    if ( not (repo.is_dirty() or len(repo.index.diff(None)) > 0)):
        self.app.do_action('details_pull_action', frame=frame, instance=self, row=row)
